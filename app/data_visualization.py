...
def generate_graph_html(data_dict, days, leg=True):
    if ("x" in data_dict and "y" in data_dict and "label" in data_dict and
            ...
            if isinstance(i, list):
                if index_y2[y.index(i)] == 0:
                    ...
                    fig.add_trace(go.Scatter(
                        x=x_dates, y=i[:days], mode='lines+markers', name=name[y.index(i)]
                    ))
                    ...
                    fig.add_trace(go.Bar(
                        x=x_dates, y=i[:days], yaxis='y2', opacity=0.4, name=name[y.index(i)]
                    ))
            else:
                fig.add_trace(go.Scatter(
                    x=x_dates, y=i[:days], mode='lines+markers', name=name[y.index(i)]
        ...
        if not leg:
            html_content = f"""
                <div class="chart-container" style="width: 100%; height: 100%; box-sizing: border-box;">
                {html_content}
                </div>
            """
        return html_content
